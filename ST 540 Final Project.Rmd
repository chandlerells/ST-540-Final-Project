...
for(i in 1:3){
  ...
  samples1 <- coda.samples(model1, variable.names = c("Y1", "beta", "gamma"), n.iter = 10000, progress.bar = "none")[[1]]
  ...
  #Generate post-burn-in samples and retain parameters of interest
  samples2 <- coda.samples(model2, variable.names = c("Y1", "beta", "gamma"), n.iter = 10000, progress.bar = "none")[[1]]
  #store predictions and find category classified the most in PPD for each prediction
  ...
  #Generate post-burn-in samples and retain parameters of interest
  samples3 <- coda.samples(model3, variable.names = c("Y1", "beta", "gamma"), n.iter = 10000, progress.bar = "none")[[1]]
  #store predictions and find category classified the most in PPD for each prediction
...
effectiveSize(samples21[, (N_test + 1):(N_test + (n_train2 * 2))])
effectiveSize(samples3[, (N_test + 1):(N_test + (n_train3 * 2))])

summary(samples1[, (N_test + 1):(N_test + (n_train1 * 2))])
```
...
```{r}
  ...
  #Generate post-burn-in samples and retain parameters of interest
  samples4 <- coda.samples(model4, variable.names = c("p"), n.iter = 10, progress.bar = "none")[[1]]
```
